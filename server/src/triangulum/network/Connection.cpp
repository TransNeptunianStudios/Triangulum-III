...
mg_connection* Connection::raw() const { return m_connection; }
...
void Connection::send_msg(const Json& json)
{
  ...
  mg_send_websocket_frame(m_connection, WEBSOCKET_OP_TEXT, json_msg.c_str(),
                          json_msg.size());
}

void Connection::set_accepted(bool is_accepted) { m_is_accepted = is_accepted; }

bool Connection::is_accepted() const { return m_is_accepted; }
